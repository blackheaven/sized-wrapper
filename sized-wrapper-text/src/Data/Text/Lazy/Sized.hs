...
instance SizedFromContainer T.Text where
  calculateSize = fromIntegral . T.length
...
fromChunks = trustedChangeOverSized $ T.fromChunks . fmap getSized
{-# INLINE fromChunks #-}
...
cycle = trustedChangeOverSized T.cycle
{-# INLINE cycle #-}
...
iterate f = trustedSized . T.iterate f
{-# INLINE iterate #-}
